...
class server {
  constructor(serverFunctions, io) {
    ...
    this.experiment_id_event = wrapper.async(this.serverFunctions.experiments.processSessionEvent, (req) => {
      return [req.params.experimentSessionId ,req.params.id,req.params.clientId, req.body];
    }, async (result) => {
      console.log (result)
      var data = await this.serverFunctions.experiments.getSessionOverview(sessionId);
